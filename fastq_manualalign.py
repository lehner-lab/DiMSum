...
parser.add_argument('-p', '--paste', action='store_true', help='Simply concatentate reads (without reverse complementing second read in pair)', required=False)
    ...
    while True:
        ...
        if len(fqr1['read_seq']) >= 64 and len(fqr2['read_seq']) >= 64:
            if paste_reads:
                ...
                #Are qualities less than specified?
                non_merge_num_bases_too_low_qual = sum([phred_dict[i]<min_qual for i in fqr1['read_qual'] + fqr2['read_qual']])
                if non_merge_num_bases_too_low_qual == 0:
                    fqo['read_eprob'] = fqr1['read_eprob'] + fqr2['read_eprob']
                    ...
                    if exp_num_read_errors < max_ee:
                        fqo['read_seq'] = fqr1['read_seq'] + fqr2['read_seq']
                        a_stats['merged_lengths'].append(len(fqo['read_seq']))
                        fqo['read_qual'] = fqr1['read_qual'] + fqr2['read_qual']
