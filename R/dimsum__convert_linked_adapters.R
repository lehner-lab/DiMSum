...
dimsum__convert_linked_adapters <- function(
  ...
  for(i in 1:dim(dimsum_meta[['exp_design']])[1]){
    ...
    if( !is.na(dimsum_meta[['exp_design']][i,"cutadapt5First"]) & !grepl("\\.\\.\\.", dimsum_meta[['exp_design']][i,"cutadapt5First"]) & !is.na(dimsum_meta[['exp_design']][i,"cutadapt3First"]) ){
    ...
    #Check if 5' constant region specified
    if( !is.na(dimsum_meta[['exp_design']][i,"cutadapt5Second"]) & !grepl("\\.\\.\\.", dimsum_meta[['exp_design']][i,"cutadapt5Second"]) & !is.na(dimsum_meta[['exp_design']][i,"cutadapt3Second"]) ){
