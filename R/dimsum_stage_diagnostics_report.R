...
dimsum_stage_diagnostics_report <- function(
  ...
  vdm_noindel <- variant_data_merge[nchar(nt_seq)==nchar(variant_data_merge[WT==T,nt_seq]),]
  #Calculate nucleotide hamming distance
  vdm_noindel[, Nham_nt := mapply(dimsum__hamming_distance, nt_seq, vdm_noindel[WT==T,nt_seq])]
  #Calculate amino acid hamming distance
  vdm_noindel[, Nham_aa := mapply(dimsum__hamming_distance, aa_seq, vdm_noindel[WT==T,aa_seq])]
  #Sample names
  ...
  if(length(input_samples)!=0){
    dimsum__sample_count_distributions_by_ntham(input_dt = vdm_noindel[data.table::between(Nham_nt,1,4),.SD,.SDcols=c(input_samples, "Nham_nt", "Nham_aa")],
  ...
  if(length(output_samples)!=0){
    dimsum__sample_count_distributions_by_ntham(input_dt = vdm_noindel[data.table::between(Nham_nt,1,4),.SD,.SDcols=c(output_samples, "Nham_nt", "Nham_aa")],
  ...
  if(length(input_samples)!=0){
    temp_dt <- vdm_noindel[,.SD,.SDcols = c(input_samples, "Nham_nt")]
    dimsum__ggpairs_binhex(
      input_dt = log10(temp_dt[data.table::between(Nham_nt,1,2),.SD,.SDcols = input_samples]+1), 
  ...
  if(length(output_samples)!=0){
    temp_dt <- vdm_noindel[,.SD,.SDcols = c(output_samples, "Nham_nt")]
    dimsum__ggpairs_binhex(
      input_dt = log10(temp_dt[data.table::between(Nham_nt,1,2),.SD,.SDcols = output_samples]+1), 
  ...
  if(length(input_samples)!=0 | length(output_samples)!=0){
    temp_dt <- vdm_noindel[,.SD,.SDcols = c(input_samples, output_samples, "Nham_nt")]
    dimsum__ggpairs_binhex(
