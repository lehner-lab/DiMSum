...
dimsum__filter_nuc_variants_coding <- function(
  ...
  ### Retain nucleotide variants with same length as WT and max 2 amino acid substitutions only
  ...
  #Retain nucleotide variants with same length as WT and max 2 amino acid substitutions only
  input_dt <- input_dt[nchar(nt_seq)==nchar(input_dt[WT==T,nt_seq]) & Nmut_aa<=2,]
  ...
  output_dt <- copy(input_dt[((Nmut_codons-Nmut_aa) == 0 | Nmut_aa == 0) & rowSums(input_dt[,input_samples,with=F]>min_input_read_count) != 0])
