...
Expected counts from 'fake' variants (due to base-call errors at a rate corresponding to the 'usearchMinQual' option) are indicated by coloured dashed lines. Bimodal distributions (or unimodal distributions not surpassing the indicated thresholds) indicate variants originating from sequencing errors likely due to a library 'bottleneck'. A minimum input count threshold should be chosen to remove such variants (see 'fitnessMinInputCountAll' option applied in Stage 5 and [DiMSum documentation](https://github.com/lehner-lab/DiMSum) for more details.).

")
```

```{r process5_image, echo=FALSE, fig.cap="", out.width = '100%', eval=doc_settings[["show_process1"]]}
...
The lower panel in the plot below shows variance of fitness scores between replicates as a function of sequencing count-based (Poisson) variance expectation (average across replicates). The DiMSum error model (red line) describes deviations from expectation in the observed variance of fitness scores (black dashed line). The diagonal green dashed line indicates the multiplicative error term contribution. The horizontal green dashed line indicates the additive error term contribution and blue dots show mean variance per bin.

A good fit of the DiMSum error model (red line) to the binned observations (blue points) indicates that the estimated parameters (upper panels) accurately describe the variation in fitness between replicates.
